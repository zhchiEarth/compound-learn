...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function exitMarket(address cTokenAddress) external returns (uint) {
        ...
        // load into memory for faster iteration
				// 6. 查找在accountAssets资产列表中index的位置，
        ...
        // copy last item in list to location of item to be removed, reduce length by 1
				// 7.复制accountAssets 资产列表中最好一项，到将要删除的那一项中去
    ...
    function redeemAllowedInternal(address cToken, address redeemer, uint redeemTokens) internal view returns (uint) {
        ...
        if (!markets[cToken].accountMembership[redeemer]) {
            return uint(Error.NO_ERROR);
				...
				// 3.判断用户在所有市场中，是否健康
				// 返回参数一 错误码
				// 返回参数二 抵押物总价值 - 债务总价值 的值，如果大于0， 代表用户是健康的
				// 返回参数三 债务总价值 - 抵押物总价值,如果大于0， 代表用户已经资不抵债了
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        if (!markets[cToken].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
				...
				// 判断用户是否进入这个市场, 没有进入就帮他进入一下，进入了市场才能做操作 
				// ，就是前端 https://app.compound.finance/， Collateral这个勾
    ...
    function seizeAllowed(
        ...
        // Shh - currently unused
        seizeTokens;
				// 判断 抵押物和借款的ctkoen是否开始
        if (!markets[cTokenCollateral].isListed || !markets[cTokenBorrowed].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
        }
				// 判断 2个市场的comptroller是否相等
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            // Read the balances and exchange rate from the cToken
						// 拿到存款，借款，当前的汇率
						// cTokenBalance = 5000  borrowBalance = 0  exchangeRateMantissa = 0.02 * 1e18
            (oErr, vars.cTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = asset.getAccountSnapshot(account);
            if (oErr != 0) { // semi-opaque error code, we assume NO_ERROR == 0 is invariant between upgrades
                return (Error.SNAPSHOT_ERROR, 0, 0);
            }

						// 获取每个CToken的抵押率是多少，最多能贷多少钱，不能超过90%
            ...
            // Calculate effects of interacting with cTokenModify
						// 如果是这次要借的资产，就需要把这次要借的资产,算入进去
            if (asset == cTokenModify) {
                // redeem effect
                // sumBorrowPlusEffects += tokensToDenom * redeemTokens
								// 把提现的钱 也算入债务中去  提现是存入ctoken  取出token
        ...
        // These are safe, as the underflow condition is checked first
				// return (Error.NO_ERROR, liqudity, shortfall)
				// liqudity  可借额度(总抵押 - 总债务)
				// shortfall 超过额度(总债务 - 总抵押)
        if (vars.sumCollateral > vars.sumBorrowPlusEffects) {
						// (Error.NO_ERROR, liqudity, 0)
            return (Error.NO_ERROR, vars.sumCollateral - vars.sumBorrowPlusEffects, 0);
        } else {
						// (Error.NO_ERROR, 0, shortfall)
     ...
     * @param cTokenBorrowed The address of the borrowed cToken   借款的资产
     * @param cTokenCollateral The address of the collateral cToken 抵押资产
     * @param actualRepayAmount The amount of cTokenBorrowed underlying to convert into cTokenCollateral tokens
     * @return (errorCode, number of cTokenCollateral tokens to be seized in a liquidation)
     */
		 // 将清算的抵押物 换算成被清算人的 token数量
    function liquidateCalculateSeizeTokens(address cTokenBorrowed, address cTokenCollateral, uint actualRepayAmount) public view returns (uint, uint) {
    ...
    function _setCollateralFactor(CToken cToken, uint newCollateralFactorMantissa) external returns (uint) {
        ...
        // Verify market is listed
				// 2.通过CToken获取 market，验证CToken市场是存在
        ...
        // If collateral factor != 0, fail if price == 0
				// 4.设置的抵押率不能为0，并且cToken的UnderlyingPrice 不等于0
    ...
    function _supportMarket(CToken cToken) external returns (uint) {
        ...
        // Note that isComped is not in active use anymore
				// 4.将当前CToken市场开启，设置isListed为true， isComped是为开启状态，抵押率为0
        markets[address(cToken)] = Market({isListed: true, isComped: false, collateralFactorMantissa: 0});

				// 5.循环遍历allMarkets，确保allMarkets不存在这个地址，然后加入到allMarkets数组，allMarkets存的全部都是CToken地址
        _addMarketInternal(address(cToken));
				// 6. 初始化市场的 Comp设置，将Comp的CToken的compSupplyState和compBorrowState的index 指数均设置为1e36
		...
		// 添加到 allMarkets， allMarkets存的全部都是CToken地址
    ...
    function _setMintPaused(CToken cToken, bool state) public returns (bool) {
        ...
        require(markets[address(cToken)].isListed, "cannot pause a market that is not listed");
				// 2.验证msg.sender是否是 PauseGuardian 或 admin，其他人不允许设置
        require(msg.sender == pauseGuardian || msg.sender == admin, "only pause guardian and admin can pause");
				// 3. 只允许admin才能将，市场从暂停状态转变成取消暂停状态。
    ...
    function setCompSpeedInternal(CToken cToken, uint supplySpeed, uint borrowSpeed) internal {
        ...
        if (compSupplySpeeds[address(cToken)] != supplySpeed) {
            ...
            emit CompSupplySpeedUpdated(cToken, supplySpeed);
				...
				// 3. 如果上次的 借款Comp的速度 等于 这次新设的挖矿速度，就不需要设置
     ...
     */
		// 更新compSupplyState的供应指数
		// 增加了几个区块 * supplySpeed / ctoken().totalSupply = compSupplyState
    ...
    // 更新用户获取的Comp
        ...
        // Calculate change in the cumulative sum of the COMP per cToken accrued
				// deltaIndex = 市场供应指数 - 上次记录的个人的供应指数
        Double memory deltaIndex = Double({mantissa: sub_(supplyIndex, supplierIndex)});

        uint supplierTokens = CToken(cToken).balanceOf(supplier);

        // Calculate COMP accrued: cTokenAmount * accruedPerCToken
				// 用户CToken 余额 * (市场供应指数 - 上次记录的个人的供应指数)
    ...
    function _setContributorCompSpeed(address contributor, uint compSpeed) public {
        ...
        // note that COMP speed could be set to 0 to halt liquidity rewards for a contributor
				// 2.更新 contributor 的 上一次记录的区块到现在区块的 累计的 Comp奖励，
     ...
     */
		// 检查是否已经被弃用
		// 判断条件 ， 市场的抵押率 == 0 && 借款功能已经停止 && 市场的储备金率 == 1e18
